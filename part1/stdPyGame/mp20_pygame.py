...
win = pygame.display.set_mode((width,height))    # set_mode 인자 튜블형태, 윈도우 640x400
pygame.display.set_caption('게임만들기')
icon = pygame.image.load('./stdPyGame/game.png')
...
while run:
    win.fill((0,0,0))   # 전체 배경을 검은색으로(배경초기화)
    ...
    if userInput[pygame.K_LEFT] and x > 10:    # 왼쪽 키보드 눌리면
        x -= vel_x # 왼쪽으로 10씩 이동
